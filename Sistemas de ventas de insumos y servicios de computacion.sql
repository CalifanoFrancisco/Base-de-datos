...
/* 12 */ SELECT      Codigo_producto,Precio_unitario FROM productos    WHERE Precio_unitario=(SELECT MAX(Precio_unitario) FROM productos);
/* 13 */ SELECT      MIN(Precio_unitario)            FROM productos;
/* 14 */ SELECT      Codigo_producto,Precio_unitario FROM productos    WHERE Precio_unitario=(SELECT MIN(Precio_unitario) FROM productos);
/* 15 */ SELECT      AVG(Precio_unitario)            FROM productos;
/* 16 */ SELECT      SUM(stock)                      FROM productos;
/* 17 */ SELECT      SUM(stock)                      FROM productos    WHERE Precio_unitario<(SELECT AVG(Precio_unitario) FROM productos);
